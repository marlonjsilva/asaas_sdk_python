...
class RequestTimeoutError(Exception):
    ...
    code = "asaas_client_request_timeout"
...
class HTTPResponseError(Exception):
    ...
    code: str = "asaas_client_response_error"
    status: int
    ...
    def __init__(self, response: httpx.Response, message: Optional[str] = None) -> None:
        if message is None:
            message = f"Request to Asaas API failed with status: {response.status_code}"
        super().__init__(message)
